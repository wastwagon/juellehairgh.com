    ...
    MailerModule.forRootAsync({
      ...
      useFactory: async (configService: ConfigService, prisma: PrismaService) => {
          ...
          const getSetting = async (key: string, defaultValue: string) => {
            try {
              ...
              const timeoutPromise = new Promise<string>((resolve) => {
                setTimeout(() => resolve(configService.get(key) || defaultValue), 1000);
              });
              
              const dbPromise = prisma.setting.findUnique({ where: { key } })
                .then(setting => setting?.value || configService.get(key) || defaultValue)
              ...
              console.warn(`⚠️ Could not fetch setting "${key}" from database, using env/default:`, error?.message || "Unknown error");
              return configService.get(key) || defaultValue;
            }
          };

          const emailProvider = await getSetting("EMAIL_PROVIDER", "smtp");
        const frontendUrl = configService.get("FRONTEND_URL") || "http://localhost:8002";
        ...
        if (emailProvider === "sendgrid") {
          ...
          if (!sendgridApiKey) {
            console.warn("SENDGRID_API_KEY not configured. Falling back to SMTP configuration.");
            const smtpHost = await getSetting("SMTP_HOST", "smtp.gmail.com");
            const smtpPort = parseInt(await getSetting("SMTP_PORT", "587"));
            const smtpUser = await getSetting("SMTP_USER", "");
            const smtpPassword = await getSetting("SMTP_PASSWORD", "");
        ...
        if (emailProvider === "mailgun") {
          ...
          console.error("❌ Email module initialization failed, using fallback config:", error?.message || "Unknown error");
          return {
            transport: {
