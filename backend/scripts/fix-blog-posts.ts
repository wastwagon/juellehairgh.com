...
Glueless lace wigs offer the perfect combination of style, comfort, and convenience. Experience the difference today!`,
    ...
    for (const post of posts) {
      const now = new Date();
      const publishedDate = post.publishedAt || now;
      
      // Ensure publishedAt is in the past or now
      const safePublishedDate = publishedDate > now ? now : publishedDate;
      ...
      console.log(`   - Published: ${post.isPublished ? 'Yes' : 'Now Yes'}`);
      console.log(`   - Published At: ${safePublishedDate.toISOString()}`);
      console.log(`   - Category: ${post.category || 'N/A'}\n`);
    }
  }

  // Verify posts are visible via API query
  ...
  console.log(`\n✅ Verification:`);
  console.log(`   - Total published posts: ${visiblePosts.length}`);
